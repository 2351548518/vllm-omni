...
![Image](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=YWEwNTc4NTEyMDFiYjZiYmM3YWYyN2ZlOGVlNjAzZDVfZjYwZGE5YTZhYWQxNmRiY2UxYWRkNDNiNzI1ZTM0MzZfSUQ6NzY2NzQ2MDUzNDE3ODg1OTk4MF8xNzg2Njc0OTU1OjE3ODY3NjEzNTVfVjM)
...
按照步骤一在HiDevLab上申请的环境已经处于容器中，这一步无需执行，请跳过
...
**说明**：为什么要用 `--no-deps`。step\-audio2 的部分传递依赖会与镜像内已锁定的 torch / torch\_npu 版本冲突，用 `--no-deps` 只装包体本身，复用镜像里的昇腾适配栈。必须 \-\-no\-deps,防止覆盖镜像内 torch/torch\_npu 把 NPU 环境装坏\)。

注意⚠️：**永不升级 torch/torch\_npu/CANN/vLLM，不过vLLM\-Omni可以随仓库自动更新**
...
参赛选手申请的环境已经安装好，在**/vllm\-workspace/vllm\-omni**下，直接执行
...
```Shell
export HF_ENDPOINT="https://hf-mirror.com"
hf download --dataset CowboyZ/seed-tts-eval seedtts_testset.tar --local_dir /workspace/seed-tts
...
```Shell
SEED_TTS_SIM_EVAL=1 vllm bench serve \
  --omni \
  --port 8091 \
  --trust-remote-code \
  --max-concurrency 1 \
  ...
  --seed-tts-file-ref-audio 指定参考音频路径
```
...
上述命令（`en` 语种、32 条、并发 1）的实测数值\(供参考\)：
...
```Shell
export HF_ENDPOINT="https://hf-mirror.com"
hf download --repo-type dataset liarliar/Daily-Omni --local-dir /workspace/user_data/datasets/liarliar/Daily-Omni
...
ACC_BENCH_RESULT_DIR=your_result_path/accuracy \
pytest -s -v \
